...
document.getElementById('undefined_period').addEventListener('change', function () {
    period_indefined();
});

function frequency() {
    const visible = document.getElementById('frequency').value == "once" ? "hidden" : "visible";

    document.getElementById('undefined_period').checked = false;
    change_visibility(document.getElementById('period_end_date_label'), visible);
    change_visibility(document.getElementById('period_end_date'), visible);
    change_visibility(document.getElementById('undefined_period'), visible);
    change_visibility(document.getElementById('undefined_period_label'), visible);
}

function period_indefined() {
    const visible = document.getElementById('undefined_period').checked ? "hidden" : "visible";
...
document.getElementById('period_start_date').addEventListener('change', function () {
    const data = new Date(this.value + 'T00:00');
    data.setFullYear(data.getFullYear() + 5);
    document.getElementById('period_end_date').value = data.toISOString().split('T')[0];
});

document.getElementById('transaction_base_id').addEventListener('change', function () {
    window.location.href = `${window.location.origin}/transacoes-planejadas/novo/?base=${this.value}`;
});
...
function addInstallmentFields(installments) {
    ...
    container.innerHTML = '';
    const period_start_date = document.getElementById('period_start_date').value;
    ...
    for (let i = 0; i < installments; i++) {
        ...
        installmentData.querySelectorAll('input').forEach(input => {
            ...
            if (field === 'installment_date') {
                input.setAttribute('value', addMonths(period_start_date, i).toISOString().slice(0, 10));
            }
        });
