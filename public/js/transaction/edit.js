document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll('.money').forEach(input => {
        input.value = applyBrlMask(parseFloat(input.value));
    });
...
const transaction_base = document.getElementById('transaction_base_id');
if (transaction_base) {
    transaction_base.addEventListener('change', function () {
...
function addInstallmentFields(installments) {
    ...
    for (let i = 0; i < installments; i++) {
        ...
        installmentData.querySelector("#installmentData").textContent = "Parcela #" + installmentNumber;
        installmentData.querySelectorAll('input').forEach(input => {
            ...
            if (field === 'installment_number') {
                input.setAttribute('value', installmentNumber);
            }
