...
@section('page_content')
    ...
    <div class="presentation">
        ...
        <div class="flex-container">
            <input type="hidden" form="form-loans-print" name="owner_id" value="{{$owner_id}}">
            <input type="hidden" form="form-loans-print" name="start_date" value="{{$start_date->format('Y-m-d')}}">
            <input type="hidden" form="form-loans-print" name="end_date" value="{{$end_date->format('Y-m-d')}}">
            <input type="hidden" form="form-loans-print" name="print" value="true">
            ...
            <div class="col_20 md_col_50 sm_col">
                <input class="button-as-input" type="submit" form="form-loans-print" value="{{ __('Print') }}" @if($owners->count() == 0) disabled @endif>
            </div>
        </div>
        <form method="get" id="form-loans" action="{{ route('reports.loans') }}" enctype="multipart/form-data"> @csrf </form>
        <form method="get" id="form-loans-print" action="{{ route('reports.loans') }}" enctype="multipart/form-data"> @csrf </form>
    </div>
