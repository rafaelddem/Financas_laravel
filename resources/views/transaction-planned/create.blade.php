...
@push('head-script')
    <script src="{{ asset('js/transaction-planned/script.js') }}" defer></script>
@endpush('head-script')
...
@section('page_content')
    ...
    <div class="presentation">
        <h2 class="card-title">{{__('Frequency')}}</h2>
        <div class="flex-container">
            <div class="col_40 md_col sm_col">
                <label for="frequency">{{__('Frequency')}}:</label>
                <select name="frequency" form="form-insert" id="frequency">
                    @foreach ($frequency as $frequency_option)
                        <option value='{{ $frequency_option }}' {{ old('frequency', 'every_month') == $frequency_option ? 'selected' : '' }}>{{ __($frequency_option) }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col_20 md_col_33 sm_col">
                <label for="period_start_date">{{__('Period Start Date')}}:</label>
                <input type="date" form="form-insert" name="period_start_date" id="period_start_date" value="{{old('period_start_date', Carbon::now()->format('Y-m-d'))}}" placeholder="{{__('Transaction Date')}}" required>
            </div>
            <div class="col_20 md_col_33 sm_col">
                <label for="period_end_date" id="period_end_date_label">{{__('Period End Date')}}:</label>
                <div class="radio-container">
                    <label class="radio-option" id="undefined_period_label"><input type="checkbox" form="form-insert" name="undefined_period" id="undefined_period">{{ __('Undefined_m') }}</label>
                </div>
            </div>
            <div class="col_20 md_col_33 sm_col">
                <input type="date" class="input-without-label" form="form-insert" name="period_end_date" id="period_end_date" value="{{old('period_end_date', Carbon::now()->addYears(5)->format('Y-m-d'))}}" placeholder="{{__('Processing Date')}}" required>
            </div>
        </div>

        <div class="separator-with-text"></div><br>

        <h2 class="card-title">{{__('Fill out the form')}}</h2>
        <div class="flex-container">
            <div class="col_75 md_col">
        ...
        <form method="post" id="form-insert" action="{{route('transaction-planned.store')}}"> @csrf </form>
    </div>
@endsection
